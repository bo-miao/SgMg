...
def interpolate(input, size=None, scale_factor=None, mode="nearest", align_corners=None):
    ...
    #     return torch.empty(input, output_shape)  # _new_empty_tensor(input, output_shape)
    # else:
...
def targets_to(targets: List[Dict[str, Any]], device):
    ...
    if "dataset_name" in targets[0]:
        return [{k: v.to(device) for k, v in t.items() if k not in
                 ["nouns", "caption", "caption2", "caption3", "dataset_name", "original_id"]} for t in targets]
    else:
        return [{k: v.to(device) for k, v in t.items() if k not in
                 ["nouns", "caption", "caption2", "caption3", "dataset_name", "original_id", "image_id"]} for t in targets]
