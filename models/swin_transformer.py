...
class SwinTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.num_layers = len(out_indices) #len(depths)  or out_indices[-1]+1
        self.embed_dim = embed_dim
        self.ape = ape
...
class Backbone(BackboneBase):
    ...
    def __init__(self, name: str,
        ...
        cfgs['out_indices'] = tuple([i for i in range(output_levels)])  # control output level
...
if __name__ == '__main__':
    model = SwinTransformer(embed_dim=96,
                    ...
                    out_indices=(0, 1, 2),
                    use_checkpoint=False)
    inputs = torch.randn(2,3,384,224)
    ...
    # out indices 0,1,2
    # torch.Size([2, 96, 96, 56])
    # torch.Size([2, 192, 48, 28])
    # torch.Size([2, 384, 24, 14])
    # out = model(inputs)
    # print(model)
    # for o in out:
    #     print(out[o].shape)

    model = Backbone(name="swin_t_p4w7")
    inputs = torch.randn(2,3,384,224)
    masks = torch.ones(2,384,224)
    out = model(NestedTensor(inputs, masks))
    for name, x in out.items():
        print(name, x.tensors.shape)
